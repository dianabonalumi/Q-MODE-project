...
The split is exhaustive and mutually exclusive by construction: every site carries exactly one pharmacophore type, so the zero in the unused channel is not a missing measurement but the assertion that the site offers no interaction potential of that kind --- \texttt{Aromatic} is routed to $h$ because a ring face presents an apolar contact surface, while \texttt{PosIonizable}/\texttt{NegIonizable} are routed to $hb$, rather than dropped to $(0,0)$, because the dominant directional contact of a charged group is a charge-assisted hydrogen bond.
Keeping the channels separate also keeps the two intensity scales of Section~\ref{sec:intensity} apart: $h$ carries Crippen LogP contributions and $hb$ carries Abraham $\alpha_2^H/\beta_2^H$ descriptors, which are dimensionally unrelated, so each channel is thresholded against its own distribution and no Crippen value is ever compared against an Abraham one.
