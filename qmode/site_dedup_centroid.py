...
def _dedup_with_centroid(features):
    ...
    for ftype, feats in by_type.items():
        ...
        for feat in feats:
            placed = False
            for group in groups:
                # usa il centroide aggiornato del gruppo invece del primo membro
                group_centroid = np.mean([m.coords for m in group], axis=0)
                if np.linalg.norm(feat.coords - group_centroid) < DIST_THRESHOLD:
                    group.append(feat)
                    placed = True
                    break
            if not placed:
